...
object MagnetPattern extends App {
  ...
  /*
    PROBLEMS:
     ...
     4 - type inference and default args
       ...
       actor.receive(?!) - compiler doesnt know which arg
   */
  ...
  def receive[R](magnet: MessageMagnet[R]): R = magnet() // only one receive method
  ...
  // 2 - lifting works :) but small catch
  trait MathLib {
    def add1(x: Int) = x + 1
  ...
  //val receiveFV = receive _
  //receiveFV(new P2PResponse) // confuses compiler
  ...
  /*
    ...
    4 - call by name doesn't work correctly (exercise - prove it!!)
   */
